package com.hartwig.oncoact.common.utils.sv;
...
public class ChrBaseRegion implements Cloneable, Comparable<ChrBaseRegion>
{
    ...
    public boolean overlaps(final ChrBaseRegion other)
    {
        ...
        return BaseRegion.positionsOverlap(Positions[SE_START], Positions[SE_END], other.Positions[SE_START], other.Positions[SE_END]);
    }

    public boolean containsPosition(int position) { return BaseRegion.positionWithin(position, start(), end()); }

    public boolean containsPosition(final String chromosome, int position)
    {
        return Chromosome.equals(chromosome) && BaseRegion.positionWithin(position, start(), end());
    }
