...
that are determined by the Hartwig pipeline. PROTECT works exclusively on a clinical database generated
by [SERVE](https://github.com/hartwigmedical/serve/blob/master/algo/README.md)
...
For small variants (SNVs and INDELs) determined by [PURPLE](https://github.com/hartwigmedical/hmftools/tree/master/purple/README.md) the
...
- If the variant falls within the range in which the evidence is applicable then the evidence is applicable if the variant mutation type
  passes the filter defined as part of the SERVE evidence rule and the combination of variants affecting that variant have a high driver
  likelihood (> 80%).
...
Evidence on amplifications/overexpressions and deletions/underexpressions is considered applicable in case a gene has been classified as
amplified or deleted by
[PURPLE](https://github.com/hartwigmedical/hmftools/tree/master/purple/README.md). In addition, a deletion is assumed to inactivate a gene
and hence evidence on
gene inactivation is considered applicable in case of a deletion.

#### Homozygous disruptions

When a gene has been homozygously disrupted according to [LINX](https://github.com/hartwigmedical/hmftools/tree/master/linx/README.md),
evidence is applicable when it has been defined
...
For matching viral presence to evidence, the interpretation
by [Virus Interpreter](https://github.com/hartwigmedical/hmftools/tree/master/virus-interpreter/README.md) is used. If virus
...
Evidence on wild-types matches only when the gene is considered as wild-type. Currently, all the matching wild-type evidence is set to
unreportable.
...
4. For every gene/event/treatment/direction combination only the highest on-label level of evidence is reported:
    ...
    - Off-label evidence is only reported in case there is no on-label evidence or the evidence level is higher than the highest on-label
      evidence.
...
- [3.0.0](https://github.com/hartwigmedical/oncoact/releases/tag/protect-3.0.0)
    - Major changes to datamodel in general (different packages, different classes etc)
    - Switch to SERVE 2.0
    - Solve bug of INC23-111 (in BizzMine)
...
- [2.3](https://github.com/hartwigmedical/hmftools/releases/tag/protect-v2.3)
    ...
    - Improve the promiscuous fusion matching to match only the evidence of that gene if a fusion with that gene is reported either in 5' or
      3'
